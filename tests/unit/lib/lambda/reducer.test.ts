...
describe("function reduce(t: Term): Term", async () => {
  ...
  describe("any strategy", async () => {
    ...
    test("reduce((λf x -> f x) x) = λx' -> x x'", async () => {
      ...
      expect(reduce(t)).toEqual(parse("λx' -> x x'"));
    });
  });
